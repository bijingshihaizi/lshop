...
class MemberController extends Controller
{
    public function register(Request $request)
    {
        ...
        if ($email == '' && $phone == ''){
            $m3_result->status = 1;
            $m3_result->message = '手机号码或者邮箱不能为空';
            return $m3_result->tooJson();
        }
        if ($password = '' || strlen($password) < 6){
            $m3_result->status = 2;
            $m3_result->message = '密码不能小于6位数';
            return $m3_result->tooJson();
        }
        if ($confirm = '' || strlen($confirm) < 6){
            $m3_result->status = 3;
            $m3_result->message = '确认密码不能小于6位数';
            return $m3_result->tooJson();
        }
        if ($password != $confirm){
            $m3_result->status = 4;
            $m3_result->message = '两次输入密码不相同';
            return $m3_result->tooJson();
        }
        // 手机号注册
        if($phone != '') {
